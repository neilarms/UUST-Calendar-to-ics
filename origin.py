...
def get_calendar_object(group_id, semester_id, week_number):
    ...
    try:
        ...
        if filtered_json_data_str:
            ...
            raise ValueError("Ни одного занятия не найдено.")
...
def put_group_id(input_group_name, semester_id):
    ...
    try:
        ...
        if matching_group_id:
            ...
            raise ValueError(f"Группа '{input_group_name}' не найдена в словаре.")
    except requests.RequestException as e:
...
def put_semester_id(input_semester):
...
def open_file(group_name, week_number):
    ...
    with open(f'Расписание_занятий_для_группы_{group_name}_на_{week_number}-ю_неделю.ics', 'w') as file:
        file.write(header)

def close_file(group_name, week_number):
    footer = '\nEND:VCALENDAR\n'
    with open(f'Расписание_занятий_для_группы_{group_name}_на_{week_number}-ю_неделю.ics', 'a') as file:
        file.write(footer)
    print(f'Файл "Расписание_занятий_для_группы_{group_name}_на_{week_number}-ю_неделю.ics" сформирован')
...
def main():
    try:
        ...
        input_semester = input(f"Введите семестр в формате \"Осенний семестр 2023/2024\": ")
        week_number = int(input("Введите номер недели: "))

        semester_id = put_semester_id(input_semester)
        group_id = put_group_id(input_group_name, semester_id)
        ...
        if calendar_object:
            ...
            end_time = time.time()
            print(f'{Fore.GREEN}Файл успешно создан. Время выполнения программы: {end_time - start_time:.2f} сек.')
